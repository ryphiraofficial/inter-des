...
import { Target, Calendar, ChevronRight, Filter, ChevronDown } from 'lucide-react';
import './Engineer.css';
import { useGetEngineerTasksQuery, useGetEngineerProjectsQuery } from '../../../store/api/productionApi';
...
const EngineerTasks = ({ isTransferred }) => {
    const navigate = useNavigate();
    const [filters,  setFilters]  = useState({ status:'All', priority:'All', stage:'All', projectId:'All' });
    const [filtersOpen, setFiltersOpen] = useState(false);
    
    const { data: tRes, isLoading: loadingTasks } = useGetEngineerTasksQuery(isTransferred ? { transferred: 'true' } : {});
    const { data: pRes, isLoading: loadingProjects } = useGetEngineerProjectsQuery();

    const tasks = tRes?.success ? tRes.data : [];
    const projects = pRes?.success ? pRes.data : [];
