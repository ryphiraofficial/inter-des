...
import { useGetPendingLeavesQuery, useUpdateLeaveStatusMutation } from '../../../../../store/api/productionApi';

const LeaveApprovals = () => {
    const [actionId, setActionId] = useState(null);
    const [comments, setComments] = useState('');

    const { data: res, isLoading: loading } = useGetPendingLeavesQuery();
    ...
    const handleAction = async (id, status) => {
        try {
            await updateStatus({ id, status, managerComments: comments }).unwrap();
            setActionId(null);
            setComments('');
        } catch (error) {
