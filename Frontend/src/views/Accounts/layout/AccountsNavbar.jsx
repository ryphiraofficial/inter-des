...
import { RefreshCw, Bell, Search, Download, Video } from 'lucide-react';
...
const AccountsNavbar = ({ onRefresh, isLoading, search, setSearch, onExport }) => {
    ...
    const notificationLogic = useNotificationLogic({ setNotifications, setUnreadCount, showNotifications, setShowNotifications, notifications });

    useEffect(() => {
        const handleClickOutside = (e) => {
            if (wrapperRef.current && !wrapperRef.current.contains(e.target)) setShowNotifications(false);
        };
    ...
    const btnBase = { height: '38px', padding: '0 16px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: 'pointer', outline: 'none', transition: 'all 0.2s' };
            ...
            <div className="accounts-navbar-right" style={{ overflow: 'visible' }}>
                ...
                {searchConfig && setSearch && (
                    <div style={{ position: 'relative', width: '100%', maxWidth: '280px', minWidth: '220px' }}>
                        ...
                        <input type="text" placeholder={searchConfig.placeholder} value={search || ''} onChange={e => setSearch(e.target.value)}
                ...
                <div className="header-notification-wrapper" ref={wrapperRef} style={{ marginRight: '8px', position: 'relative' }}>
                    <button className={`header-notification-btn ${showNotifications ? 'active' : ''}`} onClick={notificationLogic.toggleNotifications}
                        ...
                        {unreadCount > 0 && <span className="header-notification-badge" style={{ top: '-2px', right: '-2px' }}>{unreadCount > 9 ? '9+' : unreadCount}</span>}
                    </button>
                    <NotificationPopup showNotifications={showNotifications} notifications={notifications} unreadCount={unreadCount}
