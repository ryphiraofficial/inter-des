...
/**
 * Shows dimension-quantity blocks grouped by Brand for selected bands.
 * Props:
...
const EdgeBandDimensions = ({ selectedBands, quantities, setQuantities, onDeselect }) => {
    // Group selected bands by brand
    const brandGroups = useMemo(() => {
        const map = new Map();
        (selectedBands || []).forEach(b => {
            const brand = b.brand || 'Other Brand';
            if (!map.has(brand)) map.set(brand, []);
            map.get(brand).push(b);
        });
    ...
    return (
        <div style={{ marginBottom: '1.5rem', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h4 style={{ fontSize: '0.82rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0 }}>
                Dimensions & Quantities — {selectedBands.length} band{selectedBands.length > 1 ? 's' : ''} selected across {brandGroups.length} brand{brandGroups.length > 1 ? 's' : ''}
            </h4>

            {brandGroups.map(([brandName, bands]) => (
                <div key={brandName} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ background: '#4f46e5', color: 'white', padding: '3px 10px', borderRadius: '8px', fontSize: '0.78rem', fontWeight: 800, display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                            <Tag size={12} /> {brandName}
                        </span>
                        <span style={{ fontSize: '0.78rem', fontWeight: 600, color: '#64748b' }}>
                            ({bands.length} selected match{bands.length > 1 ? 'es' : ''})
                        </span>
                    </div>
